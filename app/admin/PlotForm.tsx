...
export default function PlotForm() {
  ...
  const { mode, id, code: initCode, status: initStatus, price: initPrice } = useLocalSearchParams<{
    mode: string; id?: string; code?: string; status?: string; price?: string;
  }>();
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload = { code: codeRef.current.trim(), section: sectionRef.current, row: rowRef.current, col: colRef.current, price: Number(priceRef.current) || 0, status };
      // TODO: mode === 'edit' ? api.adminUpdatePlot(id, payload) : api.adminCreatePlot(graveyardId, payload)
